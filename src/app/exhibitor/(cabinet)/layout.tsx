...
/** Закрытая зона кабинета: /exhibitor/dashboard, settings, … (не публичная витрина /exhibitor/[id]) */
export default async function ExhibitorCabinetLayout({ children }: { children: ReactNode }) {
  ...
  if (session?.role === "EXHIBITOR") {
    ...
    const supportMode =
      session.impersonatedBy && profile ? { userName: profile.name, userEmail: profile.email } : null;
